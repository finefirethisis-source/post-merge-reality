...
SUPPLY_CHAIN_ANALYZER="${SUPPLY_CHAIN_ANALYZER:-src/webscrape/run_supply_chain_analysis.py}"
INPUT_DIR="${INPUT_DIR:-src/webscrape/output}"
CLONE_DIR="${CLONE_DIR:-src/webscrape/repos}"
SUPPLY_CHAIN_OUTPUT_DIR="${SUPPLY_CHAIN_OUTPUT_DIR:-${OUTPUT_DIR:-}}"
LOG_DIR="${LOG_DIR:-src/webscrape/run_logs}"
MANIFEST_PATH="${MANIFEST_PATH:-$LOG_DIR/initial_sample_supply_chain_analysis_manifest.csv}"

COMMIT_SOURCE="${COMMIT_SOURCE:-all}"
OSV_BIN="${OSV_BIN:-osv-scanner}"
OSV_TIMEOUT="${OSV_TIMEOUT:-120}"
GIT_TIMEOUT="${GIT_TIMEOUT:-60}"
COMMIT_WORKERS="${COMMIT_WORKERS:-1}"
PROGRESS="${PROGRESS:-auto}"
FINDINGS_OUTPUT_NAME="${FINDINGS_OUTPUT_NAME:-commit_supply_chain_findings.jsonl}"
SUMMARY_OUTPUT_NAME="${SUMMARY_OUTPUT_NAME:-commit_supply_chain_summary.jsonl}"
...
if [[ ! -f "$SUPPLY_CHAIN_ANALYZER" ]]; then
  echo "Supply-chain analyzer not found: $SUPPLY_CHAIN_ANALYZER" >&2
  exit 1
fi
...
if [[ "$DRY_RUN" != "1" && ! -f "$MANIFEST_PATH" ]]; then
  printf '"index","repo","primary_language","commit_source","commit_workers","osv_bin","osv_timeout","status","exit_code","started_at","finished_at","log_file","input_path","output_path"\n' > "$MANIFEST_PATH"
fi
...
repo_output_path() {
  local repo="$1"
  if [[ -n "$SUPPLY_CHAIN_OUTPUT_DIR" ]]; then
    printf '%s/%s\n' "$SUPPLY_CHAIN_OUTPUT_DIR" "$(repo_key "$repo")"
  else
...
append_manifest_row() {
...

  {
    ...
    csv_escape "$osv_bin"; printf ','
    csv_escape "$osv_timeout"; printf ','
...
build_supply_chain_analysis_command() {
  local repo="$1"
  local -n out_cmd="$2"

  out_cmd=(
    "$PYTHON_BIN" "$SUPPLY_CHAIN_ANALYZER"
    --input-dir "$INPUT_DIR"
    --clone-dir "$CLONE_DIR"
    --repo "$repo"
    --osv-bin "$OSV_BIN"
  ...
  if [[ -n "$SUPPLY_CHAIN_OUTPUT_DIR" ]]; then
    out_cmd+=(--output-dir "$SUPPLY_CHAIN_OUTPUT_DIR")
  fi
...
run_repo() {
  ...
  local cmd
  build_supply_chain_analysis_command "$repo" cmd

  {
    ...
    echo "osv_bin=$OSV_BIN"
    echo "osv_timeout=$OSV_TIMEOUT"
  ...
  write_manifest_row_file \
    ...
    "$COMMIT_WORKERS" "$OSV_BIN" "$OSV_TIMEOUT" "$status" "$exit_code" \
...
RUN_ROWS_DIR=""
if [[ "$DRY_RUN" != "1" ]]; then
  RUN_ROWS_DIR="$(mktemp -d "$LOG_DIR/supply_chain_analysis_manifest_rows.XXXXXX")"
fi
...
while IFS=$'\t' read -r index repo primary_language; do
  ...
  log_file="$LOG_DIR/${repo//\//__}.supply_chain_analysis.log"
  ...
  if [[ "$DRY_RUN" != "1" && ! -s "$input_path/commit_file_changes.jsonl" ]]; then
    ...
    append_manifest_row \
      "$index" "$repo" "$primary_language" "$COMMIT_SOURCE" \
      "$COMMIT_WORKERS" "$OSV_BIN" "$OSV_TIMEOUT" \
  ...
  if [[ "$DRY_RUN" != "1" && "$RESUME" == "1" && -e "$output_path/$FINDINGS_OUTPUT_NAME" && -s "$output_path/$SUMMARY_OUTPUT_NAME" ]]; then
    echo "[$index] skip repo=$repo primary_language=$primary_language existing supply-chain output=$output_path"
    append_manifest_row \
      "$index" "$repo" "$primary_language" "$COMMIT_SOURCE" \
      "$COMMIT_WORKERS" "$OSV_BIN" "$OSV_TIMEOUT" \
